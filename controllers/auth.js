  ...
  try {
    ...
    const emailData = {
      ...
      html: `
        ...
        <p>Hi ${firstname},</p>
        <p>The following activation link is valid for 10 minutes...</p>
    ...
    // sendEmailWithNodemailer already handles res.json on success/failure
...
const accountActivation = (req, res) => {
  ...
  if (!token) return res.json({ message: 'Something went wrong. Try again.' });

  jwt.verify(
    token,
    process.env.JWT_ACCOUNT_ACTIVATION,
    async (err, payload) => {
      // <-- use this decoded payload
      if (err) {
        return res.status(401).json({ error: 'Expired link. Signup again' });
      }

      try {
        const { firstname, lastname, email, password } = payload; // <-- no extra decode
